...
export class HeroService {
  ...
  searchHeroes(term: string):Observable<Hero[]>{
    ...
    const name = `${this.heroUrl}/?name=${term}`
    return this.http.get<Hero[]>(name)
    .pipe(
      tap(x => x.length ? this.log(`Matching term : ${term}`) : this.log(`No matching term : ${term}`)),
      catchError(this.handleError<Hero[]>('Seach Term', []))
    );
  }
